...
wait: ## wait check until deploy complete - then you can make delete
	@sleep 3
	@while true; do \
	REQ=`kubectl -n $(DRIVER_NAMESPACE) get daemonset.apps/imagevol-flex-ds -o=jsonpath="{.status.desiredNumberScheduled}"`; \
	retcode=$$?; \
	if [ $$retcode -gt 0 ]; then \
	echo "Deploy failed."; exit 1; fi; \
	READY=`kubectl -n $(DRIVER_NAMESPACE) get daemonset.apps/imagevol-flex-ds -o=jsonpath="{.status.numberReady}"`; \
	retcode=$$?; \
	if [ $$retcode -gt 0 ]; then \
	echo "Deploy failed."; exit 1; fi; \
